"""UI components for the FingerBlaster application - Purple Theme."""
...
class MarketPanel(Static):
    ...
    def render(self) -> str:
        ...
        return f"[b #c77dff]MARKET CONTEXT[/]\n" \
               f"STRIKE: [bold #e0aaff]{self.strike}[/]\n" \
               f"ENDS  : [bold #e0aaff]{self.ends}[/]\n" \
               f"BTC   : [bold #9d4edd]${self.btc_price:,.2f}[/]\n" \
    ...
    def _calculate_delta(self) -> str:
        ...
        try:
            ...
            # Use purple shades for positive, lighter purple-red for negative
            color = "#9d4edd" if diff >= 0 else "#d63384"
        ...
        if time_left_str not in ("N/A", "EXPIRED"):
            try:
                ...
                if len(parts) == 2:
                    minutes = int(parts[0])
                    if minutes < 2:
                        return f"[bold #d63384]{self.time_left}[/]"
            except (ValueError, AttributeError):
                pass
        return f"[bold #e0aaff]{self.time_left}[/]"
...
class StatsPanel(Static):
    ...
    def render(self) -> str:
        ...
        return f"[b #c77dff]ACCOUNT STATS[/]\n" \
               f"CASH : [bold #9d4edd]${self.balance:.2f}[/]\n" \
               f"SIZE : [bold #e0aaff]${self.selected_size:.2f}[/]\n" \
               f"POS  : [bold #9d4edd]Y:{self.yes_balance:.1f}[/] | [bold #d63384]N:{self.no_balance:.1f}[/]"
...
class ProbabilityChart(Static):
    ...
    def render(self) -> str:
        ...
        # Ensure even spacing by distributing labels evenly across plot height
