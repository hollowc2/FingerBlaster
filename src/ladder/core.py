...
class LadderCore:
    ...
    def is_filled(self, price_cent: int) -> bool:
        """Check if an order at this price was recently filled (5s window)."""
        if price_cent not in self.filled_orders:
            return False
        if time.time() - self.filled_orders[price_cent] < 5.0:
            return True
    ...
    def _on_order_filled(self, side: str, size: float, price: float, order_id: str) -> None:
        ...
        try:
            ...
            if matching_orders:
                ...
                self.filled_orders[fill_price_cent] = time.time()
                logger.info(f"Marked active order at price {fill_price_cent} as filled")
            else:
    ...
    async def _get_target_token(self, side: str) -> Optional[str]:
        """Get target token ID for given side (YES=Up, NO=Down)."""
        token_map = await self.fb.market_manager.get_token_map()
        if not token_map:
            return None
        return token_map.get('Up' if side == "YES" else 'Down')
    ...
    async def place_limit_order(self, price_cent: int, size: float, side: str) -> Optional[str]:
        ...
        try:
            ...
            # Polymarket minimums: $1.00 order value, 5 shares
            if shares * target_price < 1.0:
                shares = 1.0 / target_price
            if shares < 5.0:
                shares = 5.0
            shares = round(shares, 6)

            logger.info(f"Placing {side} @ {target_price:.4f}, {shares:.2f} shares")
    ...
    def get_open_orders_for_display(self) -> List[Dict]:
        ...
        return [
            {'order_id': oid, 'price_cent': od.get('price'),
             'size': od.get('size', 0.0), 'side': od.get('side', 'YES')}
            for oid, od in self.pending_orders.items()
            if od.get('price') and 1 <= od.get('price') <= 99
        ]
