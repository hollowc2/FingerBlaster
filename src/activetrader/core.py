...
CALLBACK_EVENTS: Tuple[str, ...] = (
    ...
    'cancel_started',    # () - cancel all operation started
    'cancel_completed',  # () - cancel all operation completed
    'cancel_failed',     # (error: str) - cancel all operation failed
    'size_changed',      # (size: float) - order size changed
)
...
class PositionTracker:
    ...
    Note: Entry prices are now fetched from Polymarket Data API in position_manager.
    This class is kept for backward compatibility with other parts of the codebase.
    """
...
class FingerBlasterCore:
    ...
    def __init__(self, connector: Optional[PolymarketConnector] = None):
        ...
        # Health monitoring for stale data detection
        self._last_health_check: float = 0.0
        self._health_check_interval: float = 10.0  # Check every 10 seconds
        self._stale_data_warning_shown: bool = False
        self._auto_reconnect_attempts: int = 0
        self._max_auto_reconnect_attempts: int = 3  # Max reconnection attempts per stale period
        self._last_reconnect_time: float = 0.0
        self._reconnect_cooldown: float = 60.0  # Wait 60s between reconnection attempts
            ...
            if btc_price and isinstance(btc_price, (int, float)) and btc_price > 0:
                ...
                if len(prices) >= 2:
                    ...
                    if market:
                        ...
                        strike_val = self._parse_price_to_beat(strike_str)
    ...
    async def _recalc_price(self) -> None:
        ...
        Includes health check for stale data detection.
        """
        now = time.time()

        # Periodic health check for stale data
        ...
        if current_market_id != self._cached_market_id:
            ...
            # Reset stale warning flag for new market
    ...
    async def _check_data_health(self) -> None:
        """Check health of price data and alert if stale.

        This method detects when WebSocket updates have stopped and logs
        diagnostic information to help identify issues. It also attempts
        automatic reconnection with safety guards.
        """
        try:
            is_stale = await self.market_manager.is_data_stale()

            if is_stale:
                if not self._stale_data_warning_shown:
                    freshness_info = await self.market_manager.get_data_freshness_info()
                    seconds_since = freshness_info.get('seconds_since_update', -1)

                    logger.error(
                        f"⚠️ STALE DATA DETECTED: No order book updates for {seconds_since:.1f}s "
                        f"(threshold: {freshness_info.get('stale_threshold')}s)"
                    )
                    logger.error(
                        f"   Update counter: {freshness_info.get('update_counter')} total updates"
                    )
                    self.log_msg(
                        f"⚠️ WARNING: Price data appears stale (no updates for {seconds_since:.0f}s). "
                        "WebSocket may be disconnected."
                    )
                    self._stale_data_warning_shown = True

                    # Log WebSocket connection status
                    if self.ws_manager._ws:
                        logger.info("WebSocket connection object exists")
                    else:
                        logger.warning("WebSocket connection object is None")

                # Attempt automatic reconnection with safety guards
                await self._attempt_auto_reconnect()

            else:
                # Data is fresh, reset warning flag and reconnection counter
                if self._stale_data_warning_shown:
                    self.log_msg("✓ Price data is fresh again")
                    self._stale_data_warning_shown = False
                    # Reset reconnection counter on successful recovery
                    self._auto_reconnect_attempts = 0

        except Exception as e:
            logger.error(f"Error in data health check: {e}", exc_info=True)

    async def _attempt_auto_reconnect(self) -> None:
        """Attempt automatic WebSocket reconnection with safety guards.

        Safety features:
        - Max 3 reconnection attempts per stale period
        - 60-second cooldown between attempts
        - Only reconnects if market exists
        """
        now = time.time()

        # Check if we've exceeded max attempts
        if self._auto_reconnect_attempts >= self._max_auto_reconnect_attempts:
            if self._auto_reconnect_attempts == self._max_auto_reconnect_attempts:
                logger.warning(
                    f"Max auto-reconnect attempts ({self._max_auto_reconnect_attempts}) reached. "
                    "Manual restart may be required."
                )
                self.log_msg(
                    f"⚠️ Auto-reconnect limit reached ({self._max_auto_reconnect_attempts} attempts). "
                    "Please check network/API status."
                )
                self._auto_reconnect_attempts += 1  # Increment to avoid repeated messages
            return

        # Check cooldown period
        if now - self._last_reconnect_time < self._reconnect_cooldown:
            time_remaining = self._reconnect_cooldown - (now - self._last_reconnect_time)
            logger.debug(f"Auto-reconnect cooldown: {time_remaining:.0f}s remaining")
            return

        # Check if we have a market to reconnect to
        market = await self.market_manager.get_market()
        if not market:
            logger.debug("No market available, skipping auto-reconnect")
            return
        ...
        logger.info(
            f"Attempting automatic WebSocket reconnection "
            f"(attempt {self._auto_reconnect_attempts}/{self._max_auto_reconnect_attempts})..."
        )
        self.log_msg(
        ...
        try:
            ...
            logger.debug("Stopping WebSocket for reconnection...")
            ...
            # Restart WebSocket
            logger.debug("Starting WebSocket after reconnection...")
            await self.ws_manager.start()

            logger.info("Auto-reconnect initiated successfully")
            self.log_msg("✓ Reconnection initiated, waiting for data...")

        except Exception as e:
            logger.error(f"Auto-reconnect failed: {e}", exc_info=True)
            self.log_msg(f"⚠️ Reconnection attempt failed: {e}")
    
    async def _resolve_dynamic_price_to_beat(
        self, 
        new_market: Dict[str, Any], 
        market_start_time: pd.Timestamp
    ) -> str:
        """Resolve dynamic price to beat using RTDS or API fallback.
        ...
        Returns:
            Resolved price to beat as formatted string, or "Dynamic" if unresolved
        """
        ...
        if time_since_start <= 120:
            ...
            if chainlink_price and chainlink_price > 0:
                ...
                new_market['price_to_beat'] = strike
        ...
        if price_str and price_str != "N/A":
            ...
            new_market['price_to_beat'] = price_str
            await self.market_manager.set_market(new_market)
            return price_str
    ...
    async def _try_resolve_pending_strike(self) -> None:
        """Try to resolve a pending strike if market has started.
        
        Since Polymarket API doesn't provide the strike for dynamic markets,
        we calculate it ourselves using Chainlink price at market start time.
        """
        market = await self.market_manager.get_market()
        if not market:
            return
        
        strike = str(market.get('price_to_beat', 'N/A'))
        # Check for both "Pending" and empty/invalid values since Polymarket doesn't provide it
        if strike not in ('Pending', 'N/A', 'None', '', '--', 'Loading'):
            return
        
        market_start_time = await self.market_manager.get_market_start_time()
        if not market_start_time:
            logger.debug("Cannot resolve strike: no market start time available")
            return
        
        # Check if market has started
        now = pd.Timestamp.now(tz='UTC')
        if market_start_time.tz is None:
            market_start_time = market_start_time.tz_localize('UTC')
        
        time_since_start = (now - market_start_time).total_seconds()
        
        # Try to resolve if market has started (even if just started)
        if time_since_start >= 0:
            # Avoid multiple simultaneous resolution attempts
            if hasattr(self, '_resolving_strike') and self._resolving_strike:
                return
            
            self._resolving_strike = True
            try:
                logger.info(f"Attempting to resolve strike (market started {time_since_start:.0f}s ago)")
                # Set strike to "Loading" while resolving
                market['price_to_beat'] = "Loading"
                await self.market_manager.set_market(market)
                # Emit market update to show "Loading"
                self._emit('market_update', "Loading", market.get('end_date', 'N/A'), market.get('question', 'Market'))
                
                resolved_strike = await self._resolve_dynamic_price_to_beat(market, market_start_time)
                
                if resolved_strike and resolved_strike not in ('N/A', 'None', '', 'Pending', '--'):
                    market['price_to_beat'] = resolved_strike
                    await self.market_manager.set_market(market)
                    logger.info(f"✓ Successfully resolved strike to: ${resolved_strike}")
                    # Emit market update with resolved strike
                    ends = self._format_ends(market.get('end_date', 'N/A'))
                    market_name = (market.get('question') or 
                                  market.get('title') or 
                                  market.get('event_slug') or 
                                  'Market')
                    self._emit('market_update', resolved_strike, ends, market_name)
                else:
                    logger.warning(f"Strike resolution returned invalid value: {resolved_strike}")
            finally:
                self._resolving_strike = False
        else:
            # Market hasn't started yet
            time_until_start = -time_since_start
            if time_until_start <= 60:  # Log when close to start
                logger.debug(f"Market starts in {time_until_start:.0f}s, will resolve strike then")
    ...
    async def update_market_status(self) -> None:
        ...
        # Try to resolve pending strikes for existing markets
        if market:
            ...
            try:
                ...
                if new_market:
                    ...
                    if success:
                        ...
                        # Resolve dynamic strikes and pending strikes
                        print("[CORE] Getting market start time...", flush=True)
                        market_start_time = await self.market_manager.get_market_start_time()
                        
                        # Check if market has started (for "Pending" strikes that need resolution)
                        market_has_started = False
                        if market_start_time:
                            now = pd.Timestamp.now(tz='UTC')
                            if market_start_time.tz is None:
                                market_start_time = market_start_time.tz_localize('UTC')
                            market_has_started = now >= market_start_time
                        
                        # Resolve "Dynamic" strikes
                        if strike == "Dynamic" and market_start_time:
                            print("[CORE] Resolving dynamic strike...", flush=True)
                            # Set strike to "Loading" while resolving
                            new_market['price_to_beat'] = "Loading"
                            await self.market_manager.set_market(new_market)
                            # Emit market update to show "Loading"
                            self._emit('market_update', "Loading", new_market.get('end_date', 'N/A'), new_market.get('question', 'Market'))

                            strike = await self._resolve_dynamic_price_to_beat(new_market, market_start_time)
                        
                        # Also resolve "Pending" strikes if market has started
                        # Note: Polymarket API doesn't provide strike for dynamic markets, so we calculate it
                        elif strike in ("Pending", "N/A", "None", "", "--") and market_start_time:
                            now = pd.Timestamp.now(tz='UTC')
                            if market_start_time.tz is None:
                                market_start_time = market_start_time.tz_localize('UTC')
                            time_since_start = (now - market_start_time).total_seconds()
                            
                            # Resolve if market has started (even if just started)
                            if time_since_start >= 0:
                                print(f"[CORE] Resolving strike (market started {time_since_start:.0f}s ago)...", flush=True)
                                # Set strike to "Loading" while resolving
                                new_market['price_to_beat'] = "Loading"
                                await self.market_manager.set_market(new_market)
                                # Emit market update to show "Loading"
                                ...
                                print(f"[CORE] Market starts in {-time_since_start:.0f}s, strike will resolve then", flush=True)

                        # Ensure resolved strike is stored in market data for resolution calculation
                        # This is critical - resolution reads from market.get('price_to_beat')
    ...
    async def update_countdown(self) -> None:
        ...
        try:
            ...
            if not end_str:
                logger.debug("No end_date in market data for countdown")
                return
            ...
            # Log countdown calculation for debugging (only occasionally to avoid spam)
            if not hasattr(self, '_last_countdown_log') or (now.timestamp() - getattr(self, '_last_countdown_log', 0)) > 60:
                logger.debug(f"Countdown: end={dt_end}, now={now}, diff={total_seconds:.0f}s ({total_seconds/60:.1f} min)")
                self._last_countdown_log = now.timestamp()
            ...
            logger.error(f"Error updating countdown: {e}", exc_info=True)
    ...
    async def _show_resolution(self) -> None:
        ...
        try:
            ...
            logger.info(f"  Raw strike from market: {market.get('price_to_beat')}")
            ...
            if strike_val is None:
                ...
                logger.error(f"  Raw strike: {market.get('price_to_beat')}")
    ...
    def _parse_price_to_beat(self, strike_str: str) -> Optional[float]:
        """Parse price to beat string to float.
        
        Args:
            strike_str: Price to beat string
    ...
    def size_up(self) -> None:
        ...
        if new_size <= self.config.max_order_size:
            ...
            self.log_msg(f"Size limit reached: ${self.config.max_order_size:.2f}")
    ...
    def set_size(self, size: float) -> bool:
        """Set order size directly with validation.
        
        Args:
            size: Desired order size in USDC
            
        Returns:
            True if size was set successfully, False if validation failed
        """
        if size < self.config.min_order_size:
            self.log_msg(
                f"Size too small: ${size:.2f} "
        ...
        if size > self.config.max_order_size:
            self.log_msg(
                f"Size too large: ${size:.2f} "
                f"(maximum: ${self.config.max_order_size:.2f})"
            )
            return False
        
        self.selected_size = size
        self.log_msg(f"Size set to ${self.selected_size:.2f}")
        self._emit('size_changed', self.selected_size)
        return True
    ...
    async def flatten(self) -> None:
        ...
        # Yield control to let UI update before continuing
    ...
    async def cancel_all(self) -> None:
        ...
        # Emit cancel started callback immediately
        ...
        try:
            ...
            if success:
                ...
                error_msg = "Cancel operation returned no success"
                self.log_msg("Cancel operation failed.")
                self._emit('cancel_failed', error_msg)
        except Exception as e:
            error_msg = f"Cancel error: {e}"
            self.log_msg(error_msg)
