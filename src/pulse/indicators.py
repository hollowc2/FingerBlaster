...
class RSICalculator:
    ...
    def clone(self) -> 'RSICalculator':
        """
        Create a deep copy of this RSI calculator.

        Used for maintaining separate "live" state for intra-candle updates.

        Returns:
            New RSICalculator with identical state
        """
...
class IndicatorEngine:
    ...
    def _ensure_calculators(self, product_id: str, timeframe: Timeframe):
        ...
        if product_id not in self._vwap:
            ...
            self._current_candle_open[product_id] = {}
            self._current_candle_high[product_id] = {}
            self._current_candle_low[product_id] = {}
            self._live_rsi[product_id] = {}
        ...
        if timeframe not in self._vwap[product_id]:
            ...
            # Initialize live RSI calculator (clone of official one)
            self._live_rsi[product_id][timeframe] = RSICalculator(period=self.config.rsi_period)
    ...
    async def update_live_price(self, product_id: str, timeframe: Timeframe, live_price: float) -> Optional[IndicatorSnapshot]:
        """
        Update indicators with live price without waiting for candle close.

        This mimics TradingView's behavior by calculating indicators on a partial/incomplete
        candle that uses the current live price. This makes RSI, ADX, and other indicators
        update in real-time relative to the current price.

        Args:
            product_id: Product ID
            timeframe: Timeframe to update
            live_price: Current live price from ticker

        Returns:
            Updated indicator snapshot or None if not enough data
        """
        self._ensure_calculators(product_id, timeframe)
