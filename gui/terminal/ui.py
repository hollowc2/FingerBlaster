...
class ProbabilityChart(Static):
    ...
    def render(self) -> str:
        ...
        # Use a separate grid to track which cells are part of the line (for blue coloring)
        plot_grid = [[' ' for _ in range(plot_width)] for _ in range(plot_height)]
        line_grid = [[False for _ in range(plot_width)] for _ in range(plot_height)]
        ...
        for i in range(len(self.data) - 1):
            ...
            self._draw_line(plot_grid, line_grid, px1, py1, px2, py2, plot_width, plot_height)
        ...
        # Convert grid to string with Rich markup for cyan line characters
        lines = []
        for row_idx, row in enumerate(grid):
            line_parts = []
            i = 0
            in_cyan_segment = False
            while i < len(row):
                char = row[i]
                # Check if this is a line character (only in the plot area, not in axis labels)
                is_line_char = False
                if row_idx < plot_height and i >= plot_start_x and i < plot_start_x + plot_width:
                    plot_x = i - plot_start_x
                    if plot_x >= 0 and plot_x < plot_width and line_grid[row_idx][plot_x]:
                        is_line_char = True
                
                if is_line_char:
                    if not in_cyan_segment:
                        line_parts.append('[cyan]')
                        in_cyan_segment = True
                    line_parts.append('█')
                else:
                    if in_cyan_segment:
                        line_parts.append('[/]')
            ...
            # Close any open cyan segment at end of line
            if in_cyan_segment:
                line_parts.append('[/]')
    ...
    def _draw_line(self, grid: List[List[str]], line_grid: List[List[bool]], 
                   x1: int, y1: int, x2: int, y2: int, width: int, height: int) -> None:
        """Draw a line using Bresenham's algorithm with cyan block character."""
        dx = abs(x2 - x1)
        dy = abs(y2 - y1)
        ...
        # Character for drawing the line - using block character for visibility
        line_char = '█'  # Full block character
        
        while True:
            if 0 <= x < width and 0 <= y < height:
                grid[y][x] = line_char
                line_grid[y][x] = True  # Mark as line point for blue coloring
