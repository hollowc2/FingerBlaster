#!/bin/bash
#
# FingerBlaster Terminal UI Launcher
# 
# Starts both the FastAPI backend and Vite frontend dev server for the terminal UI.
# Usage: ./scripts/run_terminal.sh
#
...
echo -e "${CYAN}"
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║         FINGER BLASTER - Terminal UI Launcher                    ║"
echo "╚══════════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
# Check Node modules for terminal frontend
TERMINAL_DIR="gui/fingerblaster-terminal"
if [ ! -d "$TERMINAL_DIR/node_modules" ]; then
    echo -e "${GREEN}[+]${NC} Installing terminal frontend dependencies..."
    cd "$TERMINAL_DIR"
    npm install
    cd "$PROJECT_ROOT"
fi
...
echo -e "${GREEN}[+]${NC} Waiting for backend to initialize..."
sleep 3
...
if ! curl -s http://localhost:${WEB_API_PORT:-8000}/api/health > /dev/null 2>&1; then
    echo -e "${YELLOW}[!]${NC} Backend may not be ready yet. Continuing anyway..."
else
...
echo ""
echo -e "${GREEN}✓${NC} Open ${CYAN}http://localhost:3000${NC} in your browser to view the Terminal UI"
echo ""
